...
def create_messenger(*, address: Address = None, command_queue: Queue = None) -> QueueBasedMessenger:
    address = address or Address("#default-messenger")
    command_queue = command_queue if command_queue is not None else queue.Queue()
    return QueueBasedMessenger(address=address, command_queue=command_queue)
